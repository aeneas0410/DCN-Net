...
## for conditional GAN: SAR prediction (dilated densenet wo decoder)
#mode='0' # 0. training + testing (n-fold cross-validation), 1. training + testing (for designated cases) , 2. testing
#gpu_id='0' # specific gpu id ('0,1') or -1: all available GPUs, -2: cpu only
...
#optimizer='Adam'
#initial_lr='0.0001' # for cyclegan: '0.0001' according to feedback adversarial network (cvpr19) # others: '0.001'
#num_classes='1' # 2 # 3
#multi_output='0'
...
#image_modality='B1_real,B1_imag,Epsilon,Rho,Sigma' #'B1_real,B1_imag,Sigma' #'B1_mag' #'B1_real,B1_imag' #'B1','Epsilon','Rho','Sigma'
#augment_sar_data='1,2' # (on or off, number of generation (min. :2))
#trn_dim='coronal,sagittal,axial' # '': 3d, 'axial,coronal,sagittal': 2.5D
...
### gan
#approach='cgan_sar' #'cgan_sar' #'cyclegan_sar2' #'multires_net' #unet #livianet # fc_densenet_dilated # deepmedic # densenet_dilated # attention_unet #cenet
## generator
...
#g_lamda='1.0,0.1,0.1,0.1,0.001,0.001'  #'1.0,0.5,0.3,0.1,0.001,0.001' #1.0,1.0,0.5,0.3,0.1,0.001,0.001 # generator L1 loss, perceptual losses (for 1st, 2nd, and 3rd layers), peak value loss, and negative value loss
#g_adv_loss_weight='0.01' # weight for adversarial loss (cgan: 0.   01)
#g_metric='loss_total' #loss #acc #acc_dc
...
#g_activation='linear' #'tanh' # last layer activation - sigmoid: independent (probability) multi-label training, softmax: dependent single label training
...
## discriminator
#d_network='dilated_densenet' #'patch_gan' #'dilated_densenet' #patch_gan #dilated_densenet
#d_num_classes='1'
#d_metric='loss_total' #loss #acc #acc_dc
#d_loss='binary_crossentropy'
#d_activation='sigmoid'
#d_trn_patch_size='160,160,80' #for livianet 50,50,50 (32,32,32; 27,27,27); for deepmedic 64,64,64 (48,48,48);
#d_trn_output_size='20,20,10' #for livianet 32,32,32 (14,14,14; 9,9,9); for deepmedic 32,32,32 (16,16,16);
#d_trn_step_size='0,0,0'
#d_tst_patch_size='160,160,80' #for livianet 50,50,50 (32,32,32; 27,27,27); for deepmedic 64,64,64 (48,48,48);
#d_tst_output_size='20,20,10' #for livianet 32,32,32 (14,14,14; 9,9,9); for deepmedic 32,32,32 (16,16,16);
...
exclude_label_num='2,1' #exclude_label_num: 0. bg, 1.dentate, 2. interposed for exclusive training or multi-output, -1: nothing
target='sar' #'tha' #dentate # dentate,interposed
image_modality='B1_real,B1_imag' #Epsilon,Rho,Sigma' #'B1_mag' #'B1_real,B1_imag,Sigma' #'B1_mag' #'B1_real,B1_imag' #'B1','Epsilon','Rho','Sigma'
augment_sar_data='1,1' # (on or off, number of generation (min. :2))
trn_dim='axial' # '': 3d, 'coronal,sagittal,axial'': 2.5D
ensemble_weight='1.0' # 0.6,0.2,0.2 for axial,sagittal,coronal slices
bg_discard_percentage='0'
bg_value='0'
trn_patch_size='160,160,80' # 32,32,32
trn_output_size='160,160,80' # 32,32,32
...
is_new_trn_label='0'
new_label_path='None'
folder_names='sar_prediction_baseline_test_trn#23_5' #'sar_prediction_baseline_test_b1+_real_imag_sigma_fc_dense_contextnet_with_perceptual_peak_negative_loss_data_2_5D_data_aug_2_fixed_debug' #'sar_prediction_baseline_test_b1+_real_imag_fc_dense_contextnet_with_perceptual_peak_negative_loss_data_2_5D_data_aug_3' # #
dataset_path='/home/asaf/jinyoung/projects/datasets/sar/'
crop_margin='11,10,11,12,-21,-14' #'3,3,3,3,21,20' # crop size in each side (e.g., 0,0,0,0,40,40: no crop in x and y slices, crop 40 slices in each side of z axis
## gan
approach='multi_cyclegan_sar' #'single_cyclegan_sar' #'cgan_sar' #'cyclegan_sar2' #'multires_net' #unet #livianet # fc_densenet_dilated # deepmedic # densenet_dilated # attention_unet #cenet
# generator
g_network='fc_dense_contextnet' #'u_net' #u_net #fc_dense_contextnet #dilated_densenet
g_num_classes='2' # 2 # 3
g_multi_output='0'
...
g_attention_loss='0'
g_overlap_penalty_loss='0'
g_lamda='0.5,0.001,1.0,0.5,0.3,0.1,0.001,0.001,0.001,0.0005,1.0,0.001,0.5,0.3,0.1,0.001,0.001,0.001' #'0.5,0.1,1.0,0.5,0.3,0.1,0.001,0.001,0.5,1.0,0.1,0.5,0.3,0.1,0.001,0.001' # single_cyclegan_sar (fwd/bwd) '0.5,0.1,1.0,0.5,0.3,0.1,0.001,0.001,0.5,1.0,0.1,0.5,0.3,0.1,0.001,0.001'   ; multi_cyclegan_sar (3 to 1) '1.0,0.001,0.001,0.001,0.5,0.3,0.1,0.001,0.001,0.001,0.001,1.0,0.001,0.5,0.3,0.1'  #cyclegan_sar: '1.0,0.01,0.01,1.0,0.5,0.3,0.1,0.001,0.001,0.01,1.0,1.0,0.01,0.5,0.3,0.1' #cyclegan_sar2: '1.0,0.01,0.01,1.0,0.5,0.3,0.1,0.5,0.3,0.1,0.001,0.001'   #'1.0,0.5,0.3,0.1,0.001,0.001' #1.0,1.0,0.5,0.3,0.1,0.001,0.001 # generator L1 loss, perceptual losses (for 1st, 2nd, and 3rd layers), peak value loss, and negative value loss
...
g_tst_step_size='0,0,0'
# discriminator
d_network='dilated_densenet' #'patch_gan' #'dilated_densenet' #'patch_gan' #'dilated_densenet' #patch_gan #dilated_densenet
d_num_classes='1'
d_metric='loss_total' #loss #acc #acc_dc
...
--preprocess_trn $preprocess_trn --preprocess_tst $preprocess_tst --num_k_fold $num_k_fold --batch_size $batch_size \
--num_epochs $num_epochs --patience $patience --optimizer $optimizer --initial_lr $initial_lr --num_classes $num_classes \
...
--bg_discard_percentage $bg_discard_percentage --bg_value $bg_value --importance_spl $importance_spl --oversampling $oversampling \
--threshold $threshold --target $target --image_modality $image_modality --augment_sar_data $augment_sar_data \
--trn_dim $trn_dim --ensemble_weight $ensemble_weight --folder_names $folder_names \
--dataset_path $dataset_path --continue_tr $continue_tr --is_unseen_case $is_unseen_case --is_measure $is_measure \
--is_new_trn_label $is_new_trn_label --new_label_path $new_label_path --g_network $g_network --g_num_classes $g_num_classes \
...
--g_tst_output_size $g_tst_output_size --g_tst_step_size $g_tst_step_size --d_network $d_network --d_num_classes $d_num_classes \
--d_metric $d_metric --d_loss $d_loss --d_activation $d_activation --d_trn_patch_size $d_trn_patch_size \

...
from keras.layers import Input, Dropout, Reshape, Conv2D, Conv3D, concatenate, BatchNormalization, Activation, \
    AveragePooling2D, AveragePooling3D, Concatenate, MaxPooling2D, MaxPooling3D, Cropping2D, Cropping3D
...
def build_patch_gan(gen_conf, train_conf):
    ...
    # C64
    if dimension == 2:
        d = Conv2D(64, kernel_size, strides=stride_size, padding=padding_opt, kernel_initializer=kernel_init)(merged_input)
    else:
    ...
    # define model
    model = Model(inputs=input[1], outputs=[output, interm_f_lst[0], interm_f_lst[1], interm_f_lst[2]],
                  name=model_name)
    model.summary()
    if num_of_gpu > 1:
...
def build_d_dilated_densenet(gen_conf, train_conf):
    ...
    model = __generate_dilated_densenet(model_name, dimension, num_g_output, num_classes, kernel_init, padding, random_seed_num, input_shape, output_shape,
                                        input_tensor, nb_dense_block, nb_layers_per_block, growth_rate, reduction,
                                        dropout_rate, weight_decay, init_conv_filters, activation, transition_pooling,
                                        initial_kernel_size, is_densenet, is_dilated_conv,
                                        is_dilated_init_conv, dilation_rate_per_block, glam,
...
def __generate_dilated_densenet(model_name, dimension, num_g_output, num_classes, kernel_init, padding, random_seed_num, input_shape=None,
                                ...
                                dropout_rate=None, weight_decay=1e-4, init_conv_filters=48, activation='sigmoid',
    ...
    with K.name_scope('DilatedDenseNet'):
        ...
        return Model(inputs=input[1], outputs=[output, interm_f[0], interm_f[1], interm_f[2]], name=model_name)


def conv_dense_transition_down(dimension, num_classes, img_input, init_conv_filters, initial_kernel_size, kernel_init, padding,
                               random_seed_num, weight_decay, concat_axis, compression,
    ...
    if is_densenet:
        ...
        for d, f in zip(dilation_rate_lst, filter_num_lst):
            i += 1
            dilation_rate = (d, d) if dimension == 2 else (d, d, d)
            x = __conv_block(dimension, x, kernel_size, kernel_init, padding, random_seed_num, f, dropout_rate, False,

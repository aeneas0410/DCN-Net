...
def build_cyclegan_sar(gen_conf, train_conf, g_model_1, d_model_1, g_model_2, d_model_2):
    ...
    [output_d1, interm_d1_f1_out, interm_d1_f2_out, interm_d1_f3_out] = d_model_1([g_in_src, gen1_out])
    # identity element
    ...
    [output_d2, interm_d2_f1_out, interm_d2_f2_out, interm_d2_f3_out] = d_model_2([g_in_sar, gen2_out])
    ...
    tf_g_sar_real = Lambda(lambda x: x[:, :, 0])(g_out_sar_real)
    tf_gen_out = Lambda(lambda x: x[:, :, 0])(gen1_out)
    #tf_gen_out = Lambda(lambda x: x[:, :, 0])(output_b)

    model = Model([g_in_src, g_in_sar, g_out_sar_real], [output_d1, output_d2,  gen1_out, output_f, gen2_out, output_b,
                                                         interm_d1_f1_out, interm_d1_f2_out, interm_d1_f3_out,
                                                         interm_d2_f1_out, interm_d2_f2_out, interm_d2_f3_out,
                                                         g_out_sar_real, g_out_sar_real, g_out_sar_real, g_out_sar_real])

    model.summary()

    # compile model with weighting of least squares loss and L1 loss
    loss_f = [d_loss, d_loss, g_loss, g_loss, g_loss, g_loss, 'mae', 'mae', 'mae',
              'mae', 'mae', 'mae', loss_functions.local_sar_peak_loss(tf_g_sar_real, tf_gen_out),
    ...
    loss_weights_f = [adv_loss_weight, adv_loss_weight, lamda[0], lamda[1], lamda[2], lamda[3], lamda[4], lamda[5],
                      lamda[6], lamda[7], lamda[8], lamda[9], lamda[10], 0, 0, lamda[11]]
...
def build_multi_cyclegan_sar_fwd(gen_conf, train_conf, g_model_1, d_model_1, g_model_2):
    ...
    [output_d, interm_f1_out, interm_f2_out, interm_f3_out] = d_model_1(gen1_out)
    # identity element
    g_in_sar = Input(shape=g_input_shape_in_sar)
    output_id = g_model_1(g_in_sar) # B -> B   # REVISE g_model_1: 3ch -> 1ch => 1ch -> 1ch
    # forward cycle
...
def build_multi_cyclegan_sar_bwd(gen_conf, train_conf, g_model_2, d_model_2, g_model_1):
    ...
    [output_d, interm_f1_out, interm_f2_out, interm_f3_out] = d_model_2(gen2_out)
    # identity element
    g_in_src = Input(shape=g_input_shape_in_src)
    ...
    tf_g_sar_real = Lambda(lambda x: x[:, :, 0])(g_out_sar_real)
    #tf_gen_out = Lambda(lambda x: x[:, :, 0])(gen1_out)
    tf_gen_out = Lambda(lambda x: x[:, :, 0])(output_f)

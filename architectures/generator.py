...
def build_unet(gen_conf, train_conf) :
    ...
    model = __generate_unet_v2_model(model_name, dimension, num_classes, input_shape, output_shape, activation, kernel_init, concat_axis,
                                     padding_opt="same")
...
def __generate_unet_v2_model(model_name, dimension, num_classes, input_shape, output_shape, activation, kernel_init, concat_axis,
                             padding_opt):

    input = Input(shape=input_shape)

    # encoder model: C64-C128-C256-C512-C512
    e1 = encoder_block(input, 64, dimension, kernel_init, padding_opt, batchnorm=False)
    e2 = encoder_block(e1, 128, dimension, kernel_init, padding_opt)
    ...
    d6 = decoder_block(b, e2, 128, dimension, kernel_init, concat_axis, padding_opt, dropout=False)
    d7 = decoder_block(d6, e1, 64, dimension, kernel_init, concat_axis, padding_opt, dropout=False)
    # output
    if dimension == 2:
...
def encoder_block(layer_in, n_filters, dimension, kernel_init, padding_opt, batchnorm=True):
    ...
    if dimension == 2:
        ...
        g = Conv3D(n_filters, kernel_size, strides=strides, padding=padding_opt, kernel_initializer=kernel_init)(layer_in)
...
def decoder_block(layer_in, skip_in, n_filters, dimension, kernel_init, concat_axis, padding_opt, dropout=True):
    kernel_size = (4, 4) if dimension == 2 else (4, 4, 4)
    strides = (2, 2) if dimension == 2 else (2, 2, 2)
...
def __generate_fc_densenet_ms(model_name, dimension, num_classes, multi_output, output_name, attention_loss, overlap_penalty_loss,
    ...
    with K.name_scope('DenseNetFCN'):
        ...
        if global_path and local_path:
            ...
            if low_res_input:
                global_input = get_low_res_layer(dimension, img_input, kernel_init, padding, weight_decay, concat_axis,
...
def __generate_dilated_densenet(model_name, dimension, num_classes, multi_output, output_name, attention_loss, overlap_penalty_loss,
    ...
    with K.name_scope('DilatedDenseNet'):
        ...
        output = organise_output(dimension, kernel_init, concat_axis, output, None, output_shape,
                                 activation, output_name, multi_output, attention_loss)
...
def conv_dense_transition_down(dimension, img_input, init_conv_filters, initial_kernel_size, kernel_init, padding,
    ...
    for block_idx in range(nb_dense_block):
        ...
        if is_dilated_conv:
            ...
            #if dimension == 2:
            x_pool = get_low_res_layer(dimension, x, kernel_init, padding, weight_decay, concat_axis, stride=p, type='')
        ...
        if db_fp_input and not is_dilated_conv:
            ...
            img_input, img_input_activation = multi_scale_input(dimension, img_input, nb_filter, initial_kernel_size,
                                                                kernel_init, padding, block_idx, block_prefix_num, weight_decay,
                                                                concat_axis)
            x = concatenate([x, img_input_activation], axis=concat_axis)
            nb_filter += nb_filter
...
def bottleneck_block(dimension, local_f, global_f, weight_decay, concat_axis, bottleneck_nb_layers, nb_filter,
    ...
    _, _, concat_list = __dense_block(dimension, db_input, kernel_size, kernel_init, padding, random_seed_num,
                                      bottleneck_nb_layers, dilation_rate, nb_filter, growth_rate, bottleneck=bottleneck,
...
def ASPP(dimension, local_f, global_f, weight_decay, nb_filter, kernel_init, padding, random_seed_num, growth_rate,
...
def transition_up_dense_conv(dimension, multi_output, l, weight_decay, concat_axis, early_transition, nb_dense_block,
                             nb_layers, kernel_init, padding, random_seed_num, growth_rate, upsampling_type, skip_list,
    ...
    for block_idx in range(nb_dense_block):
        ...
        t = __transition_up_block(dimension, l, kernel_init, padding, nb_filters=n_filters_keep, type=upsampling_type,
    ...
    if early_transition:
        x_up = __transition_up_block(dimension, x_up, kernel_init, padding, nb_filters=nb_filter, type=upsampling_type,
...
def __conv_block(dimension, ip, kernel_size, kernel_init, padding, random_seed_num, nb_filter, bottleneck=False, dropout_rate=None,
...
def __transition_down_block(dimension, ip, nb_filter, kernel_init, padding, random_seed_num, compression=1.0, weight_decay=1e-4,
                            dropout_rate=None, block_prefix=None, transition_pooling='max', stride=2):
    '''
...
def __transition_up_block(dimension, ip, kernel_init, padding, nb_filters, type='deconv', weight_decay=1E-4,
...
def get_low_res_layer(dimension, input, kernel_init, padding, weight_decay, channel_axis, stride=2, type='max'):
    strides = (stride, stride) if dimension == 2 else (stride, stride, stride)
    if dimension == 2 :
...
def multi_scale_input(dimension, input, nb_filter, initial_kernel_size, kernel_init, padding, block_idx, block_prefix_num,
                      weight_decay, concat_axis):
    input = get_low_res_layer(dimension, input, kernel_init, padding, weight_decay, concat_axis,
                                                 stride = 2, type='max')
    if dimension == 2:

...
def local_sar_peak_loss(tf_g_sar_real, tf_gen_out):
    ...
    def loss(y_true, y_pred):
        ...
        max_g_sar_real = K.max(tf_g_sar_real, keepdims=True)
        max_gen_out = K.max(tf_gen_out, keepdims=True)
        diff_peak = K.maximum((max_g_sar_real[0,0]-max_gen_out[0,0]), 0)
...
def local_sar_mean_loss(tf_g_sar_real, tf_gen_out):
    ...
    def loss(y_true, y_pred):
        ...
        mean_g_sar_real = K.mean(tf_g_sar_real, keepdims=True)
        mean_gen_out = K.mean(tf_gen_out, keepdims=True)
        diff_mean = K.abs((mean_g_sar_real[0,0]-mean_gen_out[0,0]))
...
def local_sar_real_peak(tf_g_sar_real):

    def loss(y_true, y_pred):

        max_g_sar_real = K.max(tf_g_sar_real, keepdims=True)

        return max_g_sar_real[0,0]

    return loss


def local_sar_pred_peak(tf_gen_out):

    def loss(y_true, y_pred):

        max_gen_out = K.max(tf_gen_out, keepdims=True)

        return max_gen_out[0,0]

    return loss


def local_sar_pred_neg_loss(tf_gen_out):

    def loss(y_true, y_pred):

        min_gen_out = K.min(tf_gen_out, keepdims=True)
        abs_neg_peak = K.abs(K.minimum(min_gen_out[0,0], 0))

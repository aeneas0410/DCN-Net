...
def read_sar_dataset(gen_conf, train_conf, train_id_lst, test_id_lst):
    ...
    dataset_path = gen_conf['dataset_path']
    dataset_info = gen_conf['dataset_info'][dataset]
    dataset_fname = dataset_info['dataset_filename']
    slice_crop_size = dataset_info['margin_crop_mask']
    modality = dataset_info['image_modality']
    ...
    dataset_dict = dict()
    for df_idx in range(len(dataset_fname)-1):
        mat = loadmat(os.path.join(dataset_path, dataset_fname[df_idx]))
        dataset_dict.update(mat)

    #dataset_dict = dict(list(mat1.items()) + list(mat2.items()))
    dataset_dict.pop('__header__', None)
    dataset_dict.pop('__version__', None)
    dataset_dict.pop('__globals__', None)

    for id in range(int(len(dataset_dict)/2)):
        b1_src_name = 'B1_Ex%s' % str(id + 1)
        b1_mag_src_name = 'B1_mag_Ex%s' % str(id + 1)
        b1_real_src_name = 'B1_real_Ex%s' % str(id + 1)
        b1_imag_src_name = 'B1_imag_Ex%s' % str(id + 1)
        dataset_dict[b1_mag_src_name] = np.abs(dataset_dict[b1_src_name])
        dataset_dict[b1_real_src_name] = np.real(dataset_dict[b1_src_name])
        dataset_dict[b1_imag_src_name] = np.imag(dataset_dict[b1_src_name])

    # save mask
    dataset_dict_tissue = loadmat(os.path.join(dataset_path, dataset_fname[-1]))
    dataset_dict['Mask'] = dataset_dict_tissue['Mask']
    roi_mask_file = os.path.join(file_output_dir, 'roi_mask.nii.gz')
    if os.path.exists(roi_mask_file):
        os.remove(roi_mask_file)
    __save_sar_volume(dataset_dict['Mask'], roi_mask_file, dataset_info['format'], 'float32', is_compressed=False)
    print('saved sar roi mask data as nii.gz')
        ...
        for train_id in train_id_lst:
            ...
            for mod in modality:
                ...
                if preprocess_trn == 2 or preprocess_trn == 3 or preprocess_trn == 5:
                    ...
                    #train_src_vol_crop = normalize_image(train_src_vol_crop, [-1, 1]) # tanh : -1~1
            ...
            #if preprocess_trn == 2 or preprocess_trn == 3 or preprocess_trn == 5:
                #train_sar_vol_crop = normalize_image(train_sar_vol_crop, [-1, 1])  # tanh : -1~1
        ...
        for test_id in test_id_lst:
            ...
            for mod in modality:
                ...
                if preprocess_trn == 2 or preprocess_trn == 3 or preprocess_trn == 5:
                    ...
                    #test_src_vol_crop = normalize_image(test_src_vol_crop, [-1, 1]) # tanh : -1~1
    ...
    return train_src_lst, train_sar_lst, test_src_lst, test_sar_lst
...
def save_sar_volume(gen_conf, train_conf, test_vol, gt, sar_pred, sar_prob, ovr_pat, test_id, file_output_dir):
    ...
    file_test_patient_dir = os.path.join(file_output_dir, test_id)
    if not os.path.exists(file_test_patient_dir):
        os.makedirs(file_test_patient_dir)
...
def save_sar_volume_2_5D(gen_conf, train_conf, test_vol, gt, pred_vol_ensemble, pred_vol_total, test_id, trn_dim,
                         file_output_dir):
    dataset = train_conf['dataset']
    dataset_info = gen_conf['dataset_info'][dataset]
    path = dataset_info['path']
    ...
    pred_vol_ensemble_uncrop = crop_mask.uncrop_with_margin(pred_vol_ensemble, slice_crop_size)
    __save_sar_volume(pred_vol_ensemble_uncrop, sar_filename, dataset_info['format'], 'float32', is_compressed = False)
    print('saved sar prediction (ensemble) data as nii.gz')
    ...
    for pred_vol, dim_label in zip(pred_vol_total, trn_dim):
        ...
        pred_vol_uncrop = crop_mask.uncrop_with_margin(pred_vol, slice_crop_size)
        __save_sar_volume(pred_vol_uncrop, sar_filename, dataset_info['format'], 'float32', is_compressed = False)
        print('saved sar data (%s) as nii.gz' % dim_label)

    sar_err = np.abs(np.subtract(sar_gt, pred_vol_ensemble))
    sar_err_filename = os.path.join(sar_output_dir, 'sar_err.%s' % file_format)
    sar_err_uncrop = crop_mask.uncrop_with_margin(sar_err, slice_crop_size)
    __save_sar_volume(sar_err_uncrop, sar_err_filename, dataset_info['format'], 'float32', is_compressed = True)
    print('saved sar error data as nii.gz')

    # Create a dictionary
    dict = {}
    dict['pred'] = pred_vol_ensemble_uncrop
    dict['gt'] = sar_gt_uncrop
    dict['err'] = sar_err_uncrop
    sio.savemat(os.path.join(sar_output_dir, 'sar_prediction.mat'), dict)
    print('saved sar prediction and error data as mat')
